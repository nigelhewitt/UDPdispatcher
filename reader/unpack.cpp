...
heartbeat::heartbeat(byte* &p, int &cb)
{
	id			= PK_utf8(p, cb);
...
message* unpack(byte* &p, int &cb)
{
	try{
		...
		switch(message_type){
		case 0:
			result = new heartbeat(p, cb);
			break;
		case 1:
			result = new status(p, cb);
			break;
		case 2:
			result = new decode(p, cb);
			break;
		default:
			throw 10;
		}
